...
function dateToString(value:Date, stringFormat = 'yyyy-MM-dd') {
	return value ? format(new Date(value), stringFormat) : '';
}

export default dateToString;

export function dateDiffInMS(afterDate:Date, beforeDate:Date) {
	return differenceInMilliseconds(new Date(afterDate), new Date(beforeDate));
}

export function intToTime(i:number) {
	return `${i > 12 ? i - 12 : i}:00 ${i < 12 ? 'AM' : i === 24 ? 'AM' : 'PM'}`;
}

export function intToUTCText(i:number) {
	let utcText = 'UTC';
	if (i > 0) {
		utcText = `UTC +${i}`;
	} else if (i < 0) {
